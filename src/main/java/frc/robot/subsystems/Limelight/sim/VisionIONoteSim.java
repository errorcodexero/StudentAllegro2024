...
public class VisionIONoteSim extends VisionIOPhoton {
    ...
    public VisionIONoteSim(String name, Transform3d robotToCamera, Supplier<Pose2d> robotPoseSupplier) {
        super(name, robotToCamera);

        SimCameraProperties cameraProperties = SimCameraProperties.LL2_960_720();

        sim_ = new XeroPhotonSim(name, camera_, cameraProperties, robotToCamera_);
        sim_.enableStreamChannels(true, true);
        
        // Setup Shape of Targets
        TargetModel noteModel = new TargetModel(0.3556, 0.3556);
